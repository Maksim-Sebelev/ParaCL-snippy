...
class AstGenerator
{
  ...
  private:
    ...
    BasicNode generate_comment()
    {
        ...
        if (counter >= пасхалка_.size())
            делаем_пасхалку_ = false;
    ...
    BasicNode generate_terminal_expression()
    {
        ...
        auto &&add_expression_if_its_possible = [&](Expression id) -> void {
            if (not enabled(id))
                return;
            if ((id == Expression::VariableExpr) and name_generator_.empty())
                return;
    ...
    void update_continue_expression_probability()
    {
        ...
        auto &&remaining_depth = static_cast<double>(settings_.max_expression_depth - expression_depth_);
        auto &&total_depth = static_cast<double>(settings_.max_expression_depth);
    ...
    int calc_constant_expression(last::node::BinaryOperator const &node)
    {
        ...
        switch (node.type())
        {
        case last::node::BinaryOperator::AND:
            return left && right;
        case last::node::BinaryOperator::OR:
            return left || right;
        case last::node::BinaryOperator::ADD:
            return left + right;
        case last::node::BinaryOperator::SUB:
            return left - right;
        case last::node::BinaryOperator::MUL:
            return left * right;
        case last::node::BinaryOperator::ISEQ:
            return left == right;
        case last::node::BinaryOperator::ISNE:
            return left != right;
        case last::node::BinaryOperator::ISAB:
            return left > right;
        case last::node::BinaryOperator::ISABE:
            return left >= right;
        case last::node::BinaryOperator::ISLS:
            return left < right;
        case last::node::BinaryOperator::ISLSE:
            return left <= right;
        case last::node::BinaryOperator::DIV:
            assert(right != 0);
            return left / right; // expected that right != 0, cause we must already
                                 // modified it
        case last::node::BinaryOperator::REM:
            assert(right != 0);
            return left % right; // expected that right != 0, cause we must already
                                 // modified it
    ...
    int calc_constant_expression(last::node::UnaryOperator const &node)
    {
        ...
        switch (node.type())
        {
        case last::node::UnaryOperator::PLUS:
            return +arg;
        case last::node::UnaryOperator::MINUS:
            return -arg;
        case last::node::UnaryOperator::NOT:
            return !arg;
        default:
            break;
        }
    ...
    void modify_if_variables_enabled(BasicNode &node)
    {
        ...
        */
        ...
        auto &&set_expression_to_variable =
            last::node::create(last::node::BinaryOperator{last::node::BinaryOperator::ASGN, tmp_var, std::move(node)});

        auto &&compare_expression_with_null = last::node::create(last::node::BinaryOperator{
            last::node::BinaryOperator::ISEQ, std::move(set_expression_to_variable), std::move(zero)});

        node = last::node::create(last::node::BinaryOperator{
            last::node::BinaryOperator::ADD, std::move(compare_expression_with_null), std::move(tmp_var)});
    }
    ...
    void modify_expression_to_guareanted_do_it_not_equal_to_zero(BasicNode &node)
    {
        ...
        catch (NotConstantExpression)
        { /* skip */
        }

        if (enabled(Expression::VariableExpr) and enabled(Expression::NumberLiteralExpr) and
            enabled(Expression::BinaryOperatorExpr) and enabled(last::node::BinaryOperator::ASGN) and
            enabled(last::node::BinaryOperator::ISEQ) and enabled(last::node::BinaryOperator::ADD))
  ...
  private:
    ...
    BasicNode generate_not_scoped_statement()
    {
        ...
        auto &&add_expression_if_its_possible = [&](Statement id) -> void {
            if (not enabled(id))
                return;
            if ((id == Statement::ReturnStmt) and (not inside_function_))
                return;
  ...
  private:
    BasicNode generate_while()
    {
        ...
        if (settings_.save_while)
        {
            ...
            auto &&tmp_var = last::node::create(
                last::node::Variable{"save_from_everlasting_summ__oh_hm_i_mean_while_" + std::to_string(id++)});
            auto &&zero = last::node::create(last::node::NumberLiteral{0});
            auto &&one = last::node::create(last::node::NumberLiteral{1});
            auto &&iteration_limits =
                last::node::create(last::node::NumberLiteral{static_cast<int>(settings_.while_iterations_limit)});
            auto &&tmp_var_init = last::node::create(
                last::node::BinaryOperator{last::node::BinaryOperator::ASGN, tmp_var, std::move(zero)});

            if (too_many_statements())
                return tmp_var_init; // here while is 2, not 1 statement. so we dont
                                     // want to violate statements limit.

            auto &&tmp_var_inc = last::node::create(
                last::node::BinaryOperator{last::node::BinaryOperator::ADDASGN, std::move(tmp_var), std::move(one)});

            auto &&limit_check = last::node::create(last::node::BinaryOperator{
                last::node::BinaryOperator::ISLSE, std::move(tmp_var_inc), std::move(iteration_limits)});
            condition = last::node::create(last::node::BinaryOperator{last::node::BinaryOperator::AND,
                                                                      std::move(condition), std::move(limit_check)});

            auto &&body = generate_scope();

            auto &&while_node = last::node::create(last::node::While{std::move(condition), std::move(body)});
        ...
        return last::node::create(last::node::While{std::move(condition), std::move(body)});
    }
    ...
    BasicNode generate_if()
    {
        ...
        auto &&if_node = last::node::create(last::node::If{std::move(condition), std::move(body)});
    ...
    BasicNode generate_assign()
    {
        ...
        auto &&can_generate_function =
            enabled(Expression::FunctionDeclarationExpr) and (function_depth_ < settings_.max_function_depth);
        ...
        if (kind == 0)
            variable = name_generator_.generate_absolute_new_variable();
        else if (kind == 1)
            variable = name_generator_.generate_new_variable();
        else
            variable = name_generator_.generate_existing_variable();

        return last::node::create(
            last::node::BinaryOperator{last::node::BinaryOperator::ASGN, std::move(variable), std::move(value)});
    }
    ...
    BasicNode generate_number()
    {
        return last::node::create(last::node::NumberLiteral{
            std::uniform_int_distribution<int>(settings_.min_int_imm, settings_.max_int_imm)(random_)});
    }

    BasicNode generate_variable(unique_name_id_t id = NameGenerator::RandomExistingName)
    {
        if (name_generator_.empty())
            throw std::runtime_error("Requests generation of variable when exists no variables yet");
    ...
    BasicNode generate_expression_impl()
    {
        ...
        /* if (not unterminal_expression) - exists no unterminal expression with
           positive weights, so we can only generate terminal expression */

        auto &&unterminal_expression = generate_unterminal_expression();
        if (unterminal_expression)
            return unterminal_expression;
    ...
    BasicNode generate_binary_operator()
    {
        ...
        auto &&must_use_variable = [](last::node::BinaryOperator::BinaryOperatorT id) -> bool {
            ...
            case last::node::BinaryOperator::REMASGN:
                return true;
            default:
                break;
            }
        ...
        auto &&add_binop_if_possible = [&](last::node::BinaryOperator::BinaryOperatorT id) -> void {
            ...
            if ((weight == 0) or ((not enabled(Expression::VariableExpr)) and must_use_variable(id)))
                return;
...
#if not defined(NDEBUG)
        if (ops.size() == 0)
            throw std::runtime_error("no binop enabled, but they was requested");
#endif /* not defined(NDEBUG) */
        ...
        auto &&op_use_variable_always =
            (op == last::node::BinaryOperator::ADDASGN or op == last::node::BinaryOperator::SUBASGN or
             op == last::node::BinaryOperator::MULASGN or op == last::node::BinaryOperator::DIVASGN or
        ...
        auto &&rhs_cant_be_zero =
            ((op == last::node::BinaryOperator::DIV or op == last::node::BinaryOperator::DIVASGN) and
             settings_.save_div) or
            ((op == last::node::BinaryOperator::REM or op == last::node::BinaryOperator::REMASGN) and
             settings_.save_rem);
        ...
        return last::node::create(last::node::BinaryOperator{op, std::move(lhs), std::move(rhs)});
    }
    ...
    BasicNode generate_return()
    {
        ...
        return last::node::create(last::node::Return{generate_expression()});
    }
    ...
    BasicNode generate_function_declaration()
    {
        ...
        return last::node::create(last::node::FunctionDeclaration{"", std::move(args), std::move(body)});
    }
    ...
    BasicNode generate_function_call()
    {
        ...
        return last::node::create(last::node::FunctionCall{std::move(fn_name), std::move(args)});
    }
    ...
    BasicNode generate_function_body_expression(std::string const &arg_name)
    {
        ...
        std::array<last::node::BinaryOperator::BinaryOperatorT, 4> ops = {
            last::node::BinaryOperator::ADD, last::node::BinaryOperator::SUB, last::node::BinaryOperator::MUL,
    ...
    BasicNode generate_unary_operator()
    {
        ...
        auto &&arg = will_continue_expression() ? generate_expression_impl() : generate_terminal_expression();
...
#if not defined(NDEBUG)
        if (ops.size() == 0)
            throw std::runtime_error("no unop enabled, but they was requested");
#endif /* not defined(NDEBUG) */
        ...
        return last::node::create(last::node::UnaryOperator{op, std::move(arg)});
    }

  public:
    AstGenerator(SnippySettings const &settings)
        : settings_(settings), random_(std::random_device{}()), name_generator_(random_),
...
export last::AST generate_random_ast(SnippySettings const &settings)
{
